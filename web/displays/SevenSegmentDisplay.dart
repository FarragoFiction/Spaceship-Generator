...
class SevenSegmentDisplay extends Display {
  ...
  SvgSvgElement graphicalDisplay() {
    ...
    ret.setAttribute("width", "${45 * getNumOfDigits(maxValue)}");
...
/*
   ...
   E  F
    GG
    segments are 30 * 5, whole digit is 40 * 75
 */
class Segment {
  bool isVertical;
  int startX;
  ...
  RectElement makeGraphical(bool isLit) {
    ...
    if(isVertical) {
      ret.setAttribute("width", "5");
      ret.setAttribute("height", "30");
    } else {
      ret.setAttribute("width", "30");
      ret.setAttribute("height", "5");
    }
  ...
  static Segment make0(int startX) {
    return new Segment(startX + 5, 0, false);
  }

  static Segment make1(int startX) {
    return new Segment(startX, 5, true);
  }

  static Segment make2(int startX) {
    return new Segment(startX + 35, 5, true);
  }

  static Segment make3(int startX) {
    return new Segment(startX + 5, 35, false);
  }

  static Segment make4(int startX) {
    return new Segment(startX, 40, true);
  }

  static Segment make5(int startX) {
    return new Segment(startX + 35, 40, true);
  }

  static Segment make6(int startX) {
    return new Segment(startX + 5, 70, false);
  }
